...
|**Multi-Tasking OS**|This is the typical OS seen on computers able to preform many jobs simultaneously like Github and Spotify at the same time. To the user it appears these applications are being processed constantly but the CPU is just switch between tasks very fast| the OS on most peoples personal computers|
|**Multi-user Multi-tasking OS**|A single powerful mainframe or supercomputer is connected to many termionals all using the mainfrmae CPU, each terminal gets a slice of processor time according to the scheduling alogoritm explain in detail below.|This OS can be used on supercomputers which are much more capable than average computers meaning that the users can give it much harder problems like compplex simulations|
|**Mobile OS**|This operating system is similar to the multi-tasking OS but also comes with **utility software** that manages things specific to mobile devices such as cellular networks and wireless networks. The mobile OS is always paired with a real-time OS that is made for the devices' specific hardware|An iphone|
|**Real-time OS**|This is na OS that must:respond very quickly to any inputs; must be able to deal with many inputs simultaneosly; it must have failsafe mechanisms designed to detect and take appropriate action if hardware components fail; incorporate redundency| Fly by Wire|
|**Embedded OS**|this is an OS that preforms a small range of specific task on a single device which allows it consume less power.|Washing Machine|
