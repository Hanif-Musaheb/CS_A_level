...
Pros|Easy to use, Easy to setup, Inexpensive, suitable for single file database, all records are in one place, simple and portable, requires a simple table (relation)| Reduced data redundency and more consistency
Cons|Data redundancy (data duplication), Low security, No levels of authority, Takes more memory, searching can be inefficient|Very time consuming
