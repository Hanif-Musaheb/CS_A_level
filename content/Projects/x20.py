...
def computer_move(color,board,recursion,recursion_depth,theoretical_moves):
    ...
    for i in range(8):
        for j in range(8):
            if board[i][j]!='.':
                if board[i][j][0]==color:
                    if peice_possible_moves('{}{}'.format(j,i),enemy_color_finder('{}{}'.format(j,i),board)) !=[]:
                        peices_with_moves.append('{}{}'.format(j,i))
                        possible_moves=peice_possible_moves('{}{}'.format(j,i),enemy_color_finder('{}{}'.format(j,i),board))
