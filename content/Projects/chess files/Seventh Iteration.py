...
def move_value_finder(peice_position,move_position,color):
    possible_move_board=[]
    move_value=0
    ...
    for i in range(8):
        for j in range(8):
            if possible_move_board[i][j] != '.':
                move_value+=peice_value(possible_move_board[i][j],color)
...
def computer_move(color):
    move_values=[]#peice_position,move_position,value (format of entering into this array)
    values=[]
    moves_possible=0
    for i in range(8):
        for j in range(8):
            if board[i][j][0] == color:
                piece_possible_moves=peice_possible_moves('{}{}'.format(j,i),enemy_color_finder('{}{}'.format(j,i)))
                for q in range(len(piece_possible_moves)):
                    move_value=move_value_finder('{}{}'.format(j,i),piece_possible_moves[q],color)
                    move_values.append(['{}{}'.format(j,i),piece_possible_moves[q],move_value])
...
##    print(move_values)
    if values.index(max(values))==0:
        random_computer_output(color)
        return
    
    peice_position=move_values[values.index(max(values))][0]
    move_position=move_values[values.index(max(values))][1]
