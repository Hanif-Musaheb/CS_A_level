...
def board_stripper_yx(start_pos,position):
    ...
    for i in range(diagonal.index(position)):
        if (board[int(diagonal[(diagonal.index(position))-i-1][0])][int(diagonal[(diagonal.index(position))-i-1][1])])[0][0] != '.':#int(diagonal[(diagonal.index(position))-i-1][0])
...
output_possible_moves(horse_possible_moves(position))#board_stripper_yx(position)))
